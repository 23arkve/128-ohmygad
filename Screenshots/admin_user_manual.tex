...
\newenvironment{fieldtable}{%
  \begin{longtable}{|>{\raggedright\arraybackslash}p{2.5cm}|>{\raggedright\arraybackslash}p{1.5cm}|>{\raggedright\arraybackslash}p{2.5cm}|>{\raggedright\arraybackslash}p{7.5cm}|}
  \hline
  \rowcolor{lightgray}
...
\begin{fieldtable}
  \fieldrow{Search query}{No}{64 chars}{Type at least 1 character to see results. Press Enter to open the full search page.}
\end{fieldtable}
...
\begin{fieldtable}
  ...
  \fieldrow{Status}{Auto}{---}{Automatically calculated from the start and end dates. Possible values: \textit{Upcoming}, \textit{Today}, or \textit{Past}.}
\end{fieldtable}

\subsubsection{Event Schedule Fields}

\begin{fieldtable}
  \fieldrow{Start Date \& Time *}{Yes}{Date-time}{Must be set in the future (for new events). The event cannot start after it ends.}
  \fieldrow{End Date \& Time *}{Yes}{Date-time}{Must be after the Start Date. The minimum selectable date is automatically set to the chosen start date.}
  \fieldrow{Registration Opens *}{Yes}{Date-time}{Must be before the event ends. This is when users can begin registering.}
  \fieldrow{Registration Closes *}{Yes}{Date-time}{Must be after Registration Opens, must be before or at the event end date, and must be before the event starts. The minimum selectable date is the Registration Opens date.}
\end{fieldtable}
...
\begin{fieldtable}
  ...
  \fieldrow{Status}{Auto}{---}{Automatically calculated: \textit{Upcoming} (before open), \textit{Open} (currently accepting responses), or \textit{Closed} (past close date).}
\end{fieldtable}

\subsubsection{Availability Fields}

\begin{fieldtable}
  \fieldrow{Opens At *}{Yes}{Date-time}{When the survey becomes available to respondents. Must be in the future for new surveys. Minimum selectable date is the linked event's end date.}
  \fieldrow{Closes At *}{Yes}{Date-time}{When the survey stops accepting responses. Must be after the Opens At date.}
\end{fieldtable}
...
\begin{fieldtable}
  ...
  \fieldrow{Description}{No}{100k chars}{A long-form text area for the guideline content. This supports detailed, multi-paragraph content.}
\end{fieldtable}
...
\begin{fieldtable}
  ...
  \fieldrow{Password *}{Cond.}{8--128 chars}{Required when creating a new user. When editing, leave blank to keep the current password. Must be at least 8 characters.}
...
\begin{fieldtable}
  ...
  \fieldrow{Contact Num}{No}{11 digits}{Only digits are allowed (e.g.\ ``09123456789'').}
...
\begin{fieldtable}
  \fieldrow{Student Number *}{Yes}{9 digits}{Only digits allowed. The system automatically derives the year level from the first 4 digits (admission year).}
