...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item \textbf{Non-binary, Genderqueer, Genderfluid} — Non-binary gender identities.
...
A donut chart showing the distribution of users by gender identity (Man, Woman, Non-binary, Genderqueer, Genderfluid, Prefer not to say). This data supports the university's commitment to inclusive gender representation. Hovering over a segment displays the exact count for that category.
...
\begin{itemize}[leftmargin=1.5em]
  \item Click on any event row in the table to open the \textbf{Event Detail Modal}.
  \item At the top of the modal, click the \textbf{Registrations and Attendance} tab to switch from the event details view.
  \item The tab displays counts of total registrations and current attendance.
\end{itemize}
...
\begin{fieldtable}
  \fieldrow{Search query}{No}{---}{Type to filter by user full name, email address, or role. Searches are case-insensitive. Clearing the search box shows all users again.}
\end{fieldtable}
...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item \textbf{Role} — Alphabetically by role (Admin, Staff, Faculty, Student), ascending or descending.
\end{itemize}
...
If you have more than one page of users, pagination controls appear at the bottom of the table. Use the navigation buttons to move between pages. The display shows which users are currently shown (e.g.\ ``Showing 1–10 of 156 users'').
...
\begin{fieldtable}
  \fieldrow{College}{No}{Dropdown}{Same options as students (CS, CAC, CSS). Optional for faculty members.}
...
\begin{fieldtable}
  ...
  \fieldrow{Gender Identity *}{Yes}{Dropdown}{Options: Man, Woman, Non-binary, Genderqueer, Genderfluid, or Prefer not to say. Required to capture user's gender identity.}
\end{fieldtable}
...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item Track attendance manually only when necessary. The system automatically tracks event attendance when users register and attend events. Only manually update the Session Attendance fields if a user attended an event without registering.
  ...
  \item Archive instead of delete. Consider whether a user truly needs to be deleted or if they should be deactivated instead. Deletion is permanent and removes all history.
\end{itemize}
...
The \textbf{Guidelines Management} page lets you create, edit, view, and delete informational documents (e.g.\ ``Gender and Technology'') that are visible to all users. You can search, sort, and manage guideline content.
...
\begin{fieldtable}
  \fieldrow{Search query}{No}{---}{Type to filter by guideline title or content. Searches are case-insensitive. Clearing the search box shows all guidelines again.}
\end{fieldtable}
...
If you have more than one page of guidelines, pagination controls appear at the bottom of the table. Use the navigation buttons to move between pages. The display shows which guidelines are currently shown (e.g.\ ``Showing 1–5 of 5 guidelines'').
...
Click the \textbf{``Add Guideline''} button on the Guidelines page (or the edit icon on an existing guideline) to open the guideline form.
...
\begin{fieldtable}
  \fieldrow{Title *}{Yes}{500 chars}{Cannot be empty. Briefly describe the guideline (e.g.\ ``Gender and Technology'').}
...
\begin{itemize}[leftmargin=1.5em]
  \item Use clear, descriptive titles. Titles should convey the topic at a glance (e.g.\ ``Gender and Technology: Bridging the Digital Divide'' instead of just ``Guidelines'').
...
Click the \textbf{``Add Survey''} button on the Surveys page (or the edit icon on an existing survey) to open the survey form. The form is organized into three sections: \textit{Survey Details}, \textit{Availability}, and \textit{Questions}.
...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item Test before publishing. If possible, preview the survey in a test mode to ensure questions and choices display correctly and make sense.
