...
import {
	...
	SURVEY_STATUS_VARIANT as STATUS_VARIANT,
} from "@/lib/constants";
...
export default function SurveysPage() {
	...
	const [analyticsTarget, setAnalyticsTarget] =
		useState<SurveyFormData | null>(null);
	const [sort, setSort] = useState<{
		field: SortField;
		direction: "asc" | "desc";
	}>({ field: "open_at", direction: "desc" });
	...
	const getSurveys = async () => {
		...
		const { data, error } = await supabase
			.from("survey")
			.select(
				"id, title, description, status, event_id, open_at, close_at",
			)
	...
	useEffect(() => {
		...
		if (statusFilters.size > 0)
			result = result.filter((s) =>
				statusFilters.has(deriveStatus(s.open_at, s.close_at)),
			);
	...
	const openEditModal = async (survey: SurveyFormData) => {
		...
		const { data } = await supabase
			...
			.select(
				"id, question_text, question_type, options, is_required, order_index",
			)
