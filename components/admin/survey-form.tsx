...
import {
	...
	SURVEY_STATUS_OPTIONS as STATUS_OPTIONS,
} from "@/lib/constants";
...
export default function SurveyForm({
	...
	const [open_at, setOpenAt] = useState(
		initialData?.open_at?.slice(0, 16) ?? "",
	);
	const [close_at, setCloseAt] = useState(
		initialData?.close_at?.slice(0, 16) ?? "",
	);
	...
	const [events, setEvents] = useState<
		{ id: string; title: string; end_date?: string }[]
	>([]);
	const [selectedEventEndDate, setSelectedEventEndDate] =
	...
	const updateQuestion = (index: number, patch: Partial<SurveyQuestion>) =>
		setQuestions((prev) =>
			prev.map((q, i) => {
				...
				if (
					patch.question_type &&
					!["multiple_choice"].includes(patch.question_type)
				) {
					updated.options = [];
				}
				return updated;
	...
	const futureError =
		!isEdit && open_at && new Date(open_at) <= new Date()
	...
	const sequenceError =
		open_at &&
		close_at &&
		new Date(open_at).getTime() >= new Date(close_at).getTime()
			? "Survey close time must be after the open time."
			: null;

	const questionCountError =
		questions.length === 0 ? "At least one question is required." : null;
		...
		for (let i = 0; i < questions.length; i++) {
			...
			if (q.question_type === "multiple_choice") {
				...
				if (nonEmpty.length < 2)
					return `Question ${i + 1} needs at least 2 non-empty choices.`;

				const unique = new Set(
					nonEmpty.map((o) => o.trim().toLowerCase()),
				);
				if (unique.size !== nonEmpty.length)
					return `Question ${i + 1} has duplicate choices.`;
			}
		}
		return null;
	};
		...
		try {
			...
			if (questions.length > 0) {
				...
				const toUpdate = basePayload
					.map((p, i) =>
						questions[i].id ? { ...p, id: questions[i].id } : null,
					)
	...
	return (
		<form
			...
			className="flex h-full lg:h-auto w-full min-h-0 relative justify-center"
		>
							...
							: status === "upcoming"
								...
								: "bg-gray-100 text-gray-500"
									...
									<span className="caption text-[var(--gray)]">
										Set open/close times to determine status
									</span>
								)}
							</div>
						</div>
					...
					<div className="flex flex-col gap-2">
						...
						{show("open_at") && (openAtError || futureError) && (
							<Toast
								...
								title="Timing Error"
								message={
									openAtError || futureError || undefined
								}
							/>
						)}
						...
						<DateTimePicker
							...
							onChange={(val) => {
								setCloseAt(val);
								markTouched("close_at");
							}}
							minDate={open_at ? new Date(open_at) : new Date()}
						/>
						{show("close_at") &&
							(closeAtError || sequenceError) && (
								<Toast
									variant="error"
									title="Timing Error"
									message={
					...
					>
						<div className="border-b border-[rgba(45,42,74,0.08)] pb-2 flex items-center justify-between">
							...
							<span className="caption">
								{questions.length}/{MAX_QUESTIONS} question
						...
						{(submitAttempted ||
							...
							validationQuestionError && (
								<Toast
									...
									message={
										validationQuestionError ?? undefined
									}
								/>
							)}
						...
						{submitAttempted &&
							...
							validationQuestionError && (
								<Toast
									variant="error"
									title="Question Error"
									message={validationQuestionError}
								/>
							)}
						...
						>
							...
							{questions.length >= MAX_QUESTIONS
								? `Max ${MAX_QUESTIONS} questions reached`
								: "Add Question"}
						</button>
					</Card>
